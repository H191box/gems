...
// Layout original + tu espacio para dinero
...
// Acceso directo y volátil para seguridad de hardware GBA
...
int cargar_chunks(Chunk* slots) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        sram_read(&slots[i], SRAM_BASE + SRAM_SLOTS + i * sizeof(Chunk), sizeof(Chunk));
    }
    return (int)n;
}
...
void guardar_chunk(const Chunk* c) {
    ...
    sram_write(SRAM_BASE + SRAM_SLOTS + n * sizeof(Chunk), c, sizeof(Chunk));
    n++;
...
void sobreescribir_chunk(int slot, const Chunk* c) {
    ...
    sram_write(SRAM_BASE + SRAM_SLOTS + slot * sizeof(Chunk), c, sizeof(Chunk));
}
...
void modificar_dinero(int32_t cantidad) {
    ...
    if (total < 0) total = 0;
    uint32_t nuevo = (uint32_t)total;
    sram_write(SRAM_BASE + SRAM_DINERO, &nuevo, sizeof(uint32_t));
}
